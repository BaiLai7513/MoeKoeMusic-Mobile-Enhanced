...
- 优化在游戏页面用小窗/自由窗口/悬浮窗下，在歌单列表、搜索、搜索结果、设置/头像页、听歌识曲页面可用交互按钮补全无法使用全面屏操作部分。
- 播放歌曲功能：定时关闭、翻译，私有格式转换（对于本软件无用懒得删），下载（音质可选择）为解密格式。
- 音质选择可用，无自动降级
- 每日 VIP 静默领取，0-30S随机延时，畅听音乐。
- 加入组队领VIP功能
